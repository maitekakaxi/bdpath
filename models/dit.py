...
def fused_flex_attention(q, k, v, mask=None):
    return flex_attention(q, k, v, block_mask=mask)
fused_flex_attention = torch.compile(fused_flex_attention, fullgraph=True, mode="max-autotune-no-cudagraphs")
...
class DDiTBlock(nn.Module):
  ...
  def get_qkv(self, x, rotary_cos_sin, store_kv=False):
    ...
    #print(f"q.shape: {qkv[:,:,0].shape}, k.shape: {qkv[:,:,1].shape}, v.shape: {qkv[:,:,2].shape}")
...
class DIT(nn.Module, huggingface_hub.PyTorchModelHubMixin):
  def __init__(self, config, vocab_size: int):
    ...
    self.time_head = nn.Linear(dim, 1)
  ...
  def gen_mask(self, seqlen, block_size, attn_backend='sdpa'):
    ...
    if attn_backend == 'flex' and FLEX_ATTN_AVAILABLE:
      ...
      self.block_diff_mask = block_diff_mask(
        b=None, h=None, q_idx=torch.arange(seqlen*4)[:, None], 
  ...
  def forward(self, indices,input_time, sigma, sample_mode=False, store_kv=False):
    x_path = self.vocab_embed(indices)                     # [B, L, D]
    time_embed = self.time_proj(input_time.unsqueeze(-1))  # [B, L, D]
    x_fused = torch.cat([x_path, time_embed], dim=-1)      # [B, L, 2D]
    x = self.fuse_mlp(x_fused)                             # [B, L, D]
    ...
    with torch.amp.autocast('cuda', dtype=torch.bfloat16):
      for i in range(len(self.blocks)):
        x = self.blocks[i](
          ...
          mask=mask,
          store_kv=store_kv)
    path_logits = self.output_layer(x, t_cond)           # [B, L, vocab_size]
    time_pred = self.time_head(x).squeeze(-1)   
    if cross_attn and not sample_mode:
        path_logits = path_logits[:, :self.n]
        time_pred = time_pred[:, :self.n]
    return {
